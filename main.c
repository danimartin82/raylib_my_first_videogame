...
int main(void)
{
    ...
    Sound sound1 = LoadSound("audio/Explosion.wav");  
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        for (int i = 0; i < MAX_NUM_BALLS; i++)
        {
            if((balls[i].enable == true) && (balls[i].explosion == 0))
            {
        ...
        for (int i = 0; i < MAX_NUM_BALLS; i++)
        {
            if((balls[i].enable == true) && (balls[i].explosion != 0))
            {
                ...
                balls[i].explosion -= 1;
                int sprite = 0;
                if(balls[i].explosion >12)
                {
                    sprite = 0;
                }
                else if (balls[i].explosion >8)
                {
                    sprite = 32;
                }
                else if (balls[i].explosion >4)
                {
                    sprite = 64;
                }
                else
                ...
                DrawTextureTiled(explosion,(Rectangle){sprite,0,32,32},(Rectangle){pos.x,pos.y,64,64}, (Vector2){0,0},0.0, 2.0, WHITE);

                if(balls[i].explosion == 0)
                {
                    balls[i].enable = false;
                }
            }
...
void new_random_ball(void)
{
    ...
    int newColor=GetRandomValue(1,3);
